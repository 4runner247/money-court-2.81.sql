...
// ═══════════════════════════════════════════════════════════════════
// TradeClaw bot-engine — v4.5 "PAPER TRAIL" (2026-09-17 Thursday build;
// Data Night #6, owner voice "build as recommended" on the recommended sheet
// V1–V10, Thu 17 Sep ~20:1x Lagos). Bookkeeping + plumbing class ONLY —
// zero brain-math, zero shield-law change, zero new fire paths. Carries the
// three voted code amendments and nothing else:
//   6B STAGE-2 PROVENANCE — every final-bell shadow page now records the FATE
//      of the order it watched: broker-confirmed / ghost / held / unmatched.
//      The scout's "beautiful lie" was a false LABEL (would_trade on orders
//      that never reached the broker — 28.7% of seated pages, PASTE 18a); the
//      label now carries its own truth. Graduation gate #2 half-satisfied:
//      provenance ships; agreement-above-chance (gate #1) still absent.
//   6C PULSE HYGIENE — the MIRROR heartbeat was a raw log line fired every
//      cron beat whenever ghosts sat on the tape (~2/min; the feed's visible
//      window shrank to ~15 minutes and buried every receipt). Now throttled
//      to one line per 15 minutes. Per-trade CLOSE lines and all SHIELD /
//      SHADOW / ERROR receipts keep their own cadence, untouched.
//   6D DASHBOARD TRUTH — the status payload stamped the stale
//      pocket_option_connection balance as live ($49,976.70 vs a real
//      $49,515.89 on DN#6 night). The payload now carries the bridge's live
//      balance when reachable and ALWAYS carries a source stamp + age, so no
//      consumer can mistake a stale row for truth again.
// Rollback image: ~/v4.4-bot-engine.ts (v4.4) · previous: v4.4.2 md5 e3003f.)
//
// TradeClaw bot-engine — v4.4.2 "THE DAY IS A DAY" (2026-09-16 Wednesday build;
// owner authorisation standing: "if there is a fix it should be fixed".
// Shield class ONLY — zero brain-touch. Closes the two defects the owner's own
// PASTE 13/14 receipts exposed on Tue 15 Sep: the daily stop summed the LAST 80
// ROWS instead of the UTC day (a keyhole that shrank as volume grew — 4 hours
// wide on a 407-trade day), and its "shielded until tomorrow" was a sentence
// with no state behind it, so it un-tripped and traded ten more hours to
// -19.42 against a -10 floor. Now: day-scoped sum + the receipt as the latch.)
//
...
export async function getBotStatus() {
  ...
  // v4.5 6D DASHBOARD TRUTH (DN#6 vote V5): the connection row went stale for
  // days (DN#6 night: card said $49,976.70 while the bridge said $49,515.89 —
  // a ~$461 lie wearing a live number's clothes). From tonight the status
  // payload asks the bridge for the live balance and ALWAYS stamps the source
  // and age, so no consumer — dashboard, owner, or assistant — can mistake a
  // stale row for truth again. Bridge unreachable => the row's number still
  // shows, but labelled stale-row with its age in minutes. Read-only: one
  // GET /balance, never /trade.
  const conn = (connection ?? {}) as Record<string, unknown>;
  let balanceTruth: Record<string, unknown>;
  const bridgeUrl = process.env.POCKET_BRIDGE_URL;
  const bridgeSecret = process.env.POCKET_BRIDGE_SECRET;
  let live: { balance: number; asOf: string } | null = null;
  if (bridgeUrl) {
    try {
      const r = await fetch(`${bridgeUrl}/balance`, bridgeSecret ? { headers: { "x-bridge-secret": bridgeSecret } } : undefined);
      if (r.ok) {
        const j = (await r.json()) as { balance?: { balance?: unknown; last_updated?: unknown } };
        const b = Number(j?.balance?.balance);
        if (Number.isFinite(b)) live = { balance: b, asOf: String(j?.balance?.last_updated || new Date().toISOString()) };
      }
    } catch { /* bridge dark — the stamp below says so */ }
  }
  if (live) {
    balanceTruth = { balance: live.balance, asOf: live.asOf, source: "bridge-live" };
  } else {
    const rowAt = Date.parse(String(conn.connectedAt ?? conn.createdAt ?? "")) || 0;
    balanceTruth = {
      balance: conn.balance ?? null,
      source: "stale-row",
      rowAgeMin: rowAt ? Math.max(0, Math.round((Date.now() - rowAt) / 60000)) : null,
    };
  }
  return {
    session, openTrades, recentActivity,
    connection: { ...conn, balance: live ? live.balance : conn.balance, balanceSource: balanceTruth.source, balanceAsOf: live ? live.asOf : (conn.connectedAt ?? null) },
    balanceTruth,
  };
}
...
export async function syncBrokerResults() {
  ...
  try {
    ...
    // v4.5 6C PULSE HYGIENE (DN#6 vote V4): this heartbeat used to be a RAW
    // log line every cron beat whenever ghosts sat on the tape — ~2/min on a
    // 40-ghost tape, which shrank the feed's visible window to ~15 minutes
    // and drowned every SHIELD/SHADOW/CLOSE receipt (DN#6 Station 8 item 1:
    // "verification by feed is no longer possible"). Now one line per 15
    // minutes via the type-filtered throttle. The per-trade CLOSE lines above
    // and every SHIELD/SHADOW/ERROR receipt keep their own cadence — the
    // heartbeat steps back so the receipts can be heard.
...
export async function shieldCheck(): Promise<{ blocked: boolean; reason: string; name: string }> {
  ...
  // v4.4.2 🔑 THE KEYHOLE FIX (receipts: Tue 15 Sep, owner-run PASTE 13 + 14).
  // Until now this sum ran over `closed`, which is the LAST 80 ROWS — not the
  // day. At 132 trades a day (Mon) 80 rows ≈ most of the day, so the stop looked
  // correct. At 407 trades (Tue) 80 rows ≈ FOUR HOURS: the stop tripped at
  // 11:44 UTC on -10.16, printed "shielded until 00:00 UTC tomorrow", then
  // un-tripped the next cycle when its 4-hour keyhole improved — and traded ten
  // more hours, closing the day at -19.42. The busier the day, the weaker the
  // stop. A daily floor must be summed over THE DAY. The 80-row window stays for
  // the loss STREAK only, where recent-consecutive is exactly what is needed.
  const today = new Date().toISOString().slice(0, 10);
  let dayRows: Array<Record<string, unknown>> = [];
  try {
    dayRows = (await db.select().from(liveTrades)
      .where(sql`closed_at >= ${today}::timestamp`)
      .orderBy(desc(liveTrades.id))) as unknown as Array<Record<string, unknown>>;
  } catch { dayRows = []; } // day unreadable -> fall back to the window, honestly
  const daySource = dayRows.length ? dayRows : (closed as unknown as Array<Record<string, unknown>>);
  let dayPnl = 0;
  for (const t of daySource) {
    const st = String((t as { status?: unknown }).status || "");
    if (st !== "WIN" && st !== "LOSS" && !isUnverifiedBlind(t as { status?: unknown; strategy?: unknown })) continue;
    const ca = (t as { closedAt?: unknown }).closedAt;
    if (!ca) continue;
    if (new Date(ca as unknown as string).toISOString().slice(0, 10) !== today) continue;
    if (!String((t as { strategy?: unknown }).strategy || "").includes("LIVE")) continue;
    ...
    dayPnl += isUnverifiedBlind(t as { status?: unknown; strategy?: unknown })
      ? -Math.abs(Number((t as { stake?: unknown }).stake ?? 1))
      : Number((t as { pnl?: unknown }).pnl ?? 0);
  }
  ...
  // v4.4.2 🔒 THE LATCH. "Shielded until 00:00 UTC tomorrow" was a SENTENCE, not
  // a state: every cycle re-evaluated and a late win could quietly un-ring the
  // bell. The latch is the stop's own receipt — if a DAILY STOP line was already
  // written TODAY, the park holds until the UTC day rolls. Durable across lambda
  // restarts because the feed IS the database. No schema change, no new table,
  // no brain-touch: the receipt is the state.
  let trippedToday = false;
  try {
    const prior = await db.select().from(botActivity)
      .where(sql`created_at >= ${today}::timestamp and message like '%DAILY STOP-LOSS%'`)
      .limit(1);
    trippedToday = prior.length > 0;
  } catch { trippedToday = false; }
  if (trippedToday) {
    return { blocked: true, name: "daily-stop", reason: `🛡️ DAILY STOP-LOSS (latched): the -$${Math.abs(SHIELD.DAILY_STOP_USD)} floor was hit earlier today — the park HOLDS until 00:00 UTC tomorrow, a later recovery to $${dayPnl.toFixed(2)} does not un-ring the bell` };
  }
...
// v4.5 6B STAGE-2 PROVENANCE (DN#6 vote V3): the fate of the order a shadow
// page watched. The scout's "beautiful lie" (owner's words) was a false LABEL:
// would_trade=true written over orders that never reached the broker — 28.7%
// of seated pages (PASTE 18a). From tonight the label carries its own truth:
//   held             — no order was ever attempted (a guard/gate held it)
//   broker-confirmed — a [PO:…] receipt row exists for it on the ledger
//   ghost            — the ledger row carries [PO-NONE]: the order never landed
//   unmatched        — honest "don't know": no ledger row in the ±120 s window
// Read-only against live_trades; the notebook write happens in settleShadows.
async function shadowFate(row: Record<string, unknown>): Promise<string> {
  if (!row.would_trade) return "held";
  const createdMs = Date.parse(String(row.created_at)) || 0;
  if (!createdMs) return "unmatched";
  const stamp = (ms: number) => new Date(ms).toISOString().replace("T", " ").slice(0, 19);
  const lo = stamp(createdMs - 120e3);
  const hi = stamp(createdMs + 120e3);
  let rows: Array<Record<string, unknown>> = [];
  try {
    rows = (await db.select().from(liveTrades)
      .where(sql`opened_at >= ${lo}::timestamp and opened_at <= ${hi}::timestamp`)) as unknown as Array<Record<string, unknown>>;
  } catch { return "unmatched"; } // ledger unreadable -> say "don't know", honestly
  const sym = String(row.symbol || "");
  const dir = String(row.direction || "");
  const mine = rows.filter((t) => String((t as { symbol?: unknown }).symbol || "") === sym &&
    String((t as { direction?: unknown }).direction || "") === dir &&
    String((t as { strategy?: unknown }).strategy || "").includes("LIVE"));
  if (!mine.length) return "unmatched";
  const strat = String((mine[0] as { strategy?: unknown }).strategy || "");
  if (strat.includes("[PO-NONE]")) return "ghost";
  if (strat.includes("[PO")) return "broker-confirmed";
  return "unmatched";
}

// v4.5: exported for the behavioural harness ONLY (same disclosure as v4.4.1's
// shieldCheck): a pure notebook grader, zero trade-touch, zero side effects
// beyond its own grade writes. Production call site unchanged.
  ...
  const jobs = due.map((row) => (async () => {
    ...
    if (px == null) {
      ...
      if (ring5 && ageSec > SHADOW_NOPRICE_MIN * 60) {
        const fateNP = await shadowFate(row);
        try { await db.execute(sql`update shadow_signals set settled = true, settled_at = now(), exit_price = null, result = 'NO-PRICE', fate = ${fateNP} where id = ${id} and settled = false`); }
        catch { try { await db.execute(sql`update shadow_signals set settled = true, settled_at = now(), exit_price = null, result = 'NO-PRICE' where id = ${id} and settled = false`); } catch {} }
      }
      return;
    }
    ...
    // v4.5 6B: the final bell records the order's FATE alongside the verdict.
    const fate = ring5 ? await shadowFate(row) : null;
    try {
      if (ring5) {
        try {
          await db.execute(sql`update shadow_signals set settled = true, settled_at = now(), exit_price = ${px}, result = ${result}, fate = ${fate} where id = ${id} and settled = false`);
        } catch {
          // the fate column ships via owner paste PASTE 25; until it exists
          // the grade must still land — retry without provenance, honestly.
    ...
    if (ring5 && row.would_trade) {
      ...
      await logActivity("SHADOW", result === "WIN" ? "SUCCESS" : result === "LOSS" ? "WARNING" : "INFO",
        `🕵️ SCOUT GRADED ${symbol} ${tf} ${dir} — virtual ${result} at 5m (${entry} → ${px}) · ⏱️ 3m:${sw3} 4m:${sw4} · fate ${fate} — the seat ${result === "WIN" ? "WOULD have WON ✅" : result === "LOSS" ? "WOULD have LOST ❌" : "would have gone FLAT ➖"}`,
        { scout: true, symbol, timeframe: tf, direction: dir, entry, exit: px, result, fate, bell3: row.result_3m ?? null, bell4: row.result_4m ?? null });
    }
  })());
